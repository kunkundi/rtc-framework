...
- `client/apps/`：RTC 操作端和边缘端正式应用入口。
- `client/modules/`：运行时、边缘端、摄像头、视觉和车辆业务的可复用客户端模块。
...
- 模块公共头文件放在 `include/<命名空间>/`，跨模块包含应使用 `rtc_runtime/...`、`rtc_edge/...`、`rtc_dual_camera/...`、`rtc_vision/...` 或 `rtc_vehicle/...` 前缀。
