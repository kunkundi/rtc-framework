...
struct VehicleControlModuleOptions {
  uint32_t watchdog_ms = vts_rtc::control::kDefaultDriveWatchdogMs;
...
class VehicleControlModule {
 public:
  ...
  VehicleControlModule(IndustrialControlInterface* industrial_control,
  ...
  vts_rtc::control::VehicleGear active_gear() const { return active_gear_; }
 ...
 private:
  ...
  void ProcessDriveCommand(const vts_rtc::control::Envelope& envelope,
                           uint64_t now_ms);
  ...
  void SendEventAck(RtcSessionId remote_sessionid,
                    ...
                    const IndustrialCommandResult& result);
  ...
  vts_rtc::control::VehicleGear active_gear_ =
      vts_rtc::control::VehicleGear::Neutral;
