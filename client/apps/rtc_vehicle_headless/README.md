...
- `client/apps/rtc_vehicle_headless/main.cpp`：解析参数，创建各子模块，统一管理启动、循环和关闭顺序。
- `client/modules/vehicle/`：车辆摄像头、控制接收和工控机接口。
...
然后在 `main.cpp` 中将占位实现替换为具体实现。所有方法都在车辆主线程调用，可以在实现中接入 CAN、串口、TCP 或厂商 SDK。`SendStop()` 必须实现为本地安全停车操作。
