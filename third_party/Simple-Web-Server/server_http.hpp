...
namespace SimpleWeb {
  ...
  public:
    ...
    public:
      ...
      // Notice: added by zhujian
      std::size_t argument_size() noexcept {
        return path_match.size() >= 1 ? path_match.size() - 1 : 0;
      }

      // Notice: added by zhujian
      std::string argument_at(std::size_t index) noexcept {
        return index + 1 > this->argument_size() ? "" : (path_match.begin() + 1 + index)->str();
      }
    };
