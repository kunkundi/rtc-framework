...
#### Windows平台
1. 打开根目录下的 `run_xmake.bat`，按需设置环境变量 `QTDIR`（脚本会自动把 `QTDIR\\bin` 加入 `PATH`）
2. 运行 `run_xmake.bat`，会分别构建Debug/Release并安装到 `install` 目录
3. 习惯使用IDE开发，可执行 `xmake project -k vsxmake` 生成Visual Studio工程

#### Linux平台
1. 通用Linux环境执行 `./run_xmake.sh`
2. Jetson(aarch64)环境执行 `./run_xmake_aarch64.sh`（如Qt不在系统PATH，可先设置 `QT_DIR`）
3. 构建产物默认输出到 `build/runtime`，安装输出到 `install`
4. 习惯使用IDE开发，可执行 `xmake project -k compile_commands` 生成 `compile_commands.json`
