...
- `src/` 是公开 API 的实现。
- `src/internal/` 是内部采集、转换和拼接实现，不建议外部直接依赖。
...
if (frames->WaitNext(&frame, std::chrono::milliseconds(100)) &&
    frame.format == rtc_dual_camera::ImagePixelFormat::kI420 &&
    !frame.empty()) {
  // frame.data / frame.data_size 是拼接后的 I420 图像。
}
```
...
std::thread rtc_consumer([&] {
  while (!stop_requested.load()) {
    ...
    // 将 frame.data / frame.data_size 发送到 RTC。
  }
});
