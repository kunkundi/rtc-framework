...
typedef enum RtcMediaSourceType { Rtc = 0, SRS } RtcMediaSourceType;

typedef enum RtcRoomOperation { RoomNew, RoomDelete } RtcRoomOperation;
...
typedef enum RtcMediaChannelType { Video = 0, Audio, data } RtcMediaChannelType;
...
typedef void (*DataChannelStateHandler)(RtcSessionId, RtcDataChannelLabel,
                                        RtcDataChannelState);
// RtcServerConnectionState
...
typedef void (*RecvAudioFrameHandler)(RtcSessionId, RtcAudioSourceId,
                                      RtcMediaSourceType, size_t, size_t,
                                      size_t, size_t, const void*, size_t);
// RtcVideoSourceId, RtcMediaSourceType, width, height, dimension,
// video_data, video_data_size
typedef void (*RecvFrameHandler)(RtcSessionId, RtcVideoSourceId,
                                 RtcMediaSourceType, size_t, size_t, size_t,
 ...
 */
RTC_API RtcErrorCode
RtcInitAgent(const char* config_filepath, RoomHandler room_handler,
             P2PStateHandler P2P_state_handler,
             DataChannelStateHandler datachannel_state_handler,
             ServerConnectionStateHandler serverconnection_state_handler,
             RecvMessageHandler recv_msg_handler,
 ...
 *   @retval RtcErrorCode::AgentNotInited 获取设备失败，因为Rtc
 * Agent未成功初始化
 */
 ...
 * @return void
 * @attention
 * RtcGetVideoDevices函数成功时，必须调用该函数释放房间资源，否则存在内存泄漏
 */
 ...
 * @param max_retransmits
 * 不可靠模式下消息允许尝试重发的最大次数，若为-1，则确保可靠
 * @return 函数是否执行成功
 *   @retval RtcErrorCode::OK 增加数据通道成功
 *   @retval RtcErrorCode::AgentNotInited 增加数据通道失败，因为Rtc
 * Agent未成功初始化
...
RTC_API RtcErrorCode RtcAddDataChannel(RtcDataChannelLabel label,
                                       RtcPriorityType priority, bool ordered,
                                       int max_retransmits);
 ...
 * @attention 1.
 * 函数参数必须与RtcSendAudioFrame函数和RecvAudioFrameHandler回调函数的audio_sourceid相一致
 *            2. 针对WEB平台，当采用Unified
 * Plan形式的SDP，接收端的MediaStreamTrack的id域是唯一的GUID，
 *               并不具有业务含义，所以SDK约定一个track只属于一个stream，同时track和stream的label值相同，
 *               通过访问接收端的MediaStream的id域可以对AudioTrack进行业务区分
 */
RTC_API RtcErrorCode RtcAddExternalAudioSource(RtcAudioSourceId audio_sourceid,
 ...
 */
RTC_API RtcErrorCode RtcAddDeviceVideoSource(
    size_t device_index, const RtcVideoDeviceCapability* device_capability,
    RtcPriorityType priority);

/**
 * @brief
 * 增加来自外部的视频源（图像帧可能来自视频文件，或者是外部程序读取的摄像头捕获帧）
 *
 ...
 * @attention 1.
 * 函数参数必须与SendFrame函数和RecvFrameHandler回调函数的video_sourceid相一致
 *            2. 针对WEB平台，当采用Unified
 * Plan形式的SDP，接收端的MediaStreamTrack的id域是唯一的GUID，
 *               并不具有业务含义，所以SDK约定一个track只属于一个stream，同时track和stream的label值相同，
 ...
 * @return void
 * @attention
 * RtcQueryRoom函数成功时，必须调用该函数释放房间资源，否则存在内存泄漏
 */
 ...
 * @return void
 * @attention
 * QueryRooms函数成功时，必须调用该函数释放所有房间资源，否则存在内存泄漏
 */
 ...
 *   @retval RtcErrorCode::AgentAlreadyInRoom 打开失败，Rtc
 * Agent已经在房间中，无法打开房间
 */
 ...
 *   @retval RtcErrorCode::AgentAlreadyInRoom 加入失败，Rtc
 * Agent已经在房间中，无法加入房间
 */
...
RTC_API RtcErrorCode RtcSendData(RtcSessionId remote_sessionid,
                                 RtcDataChannelLabel channel_label,
