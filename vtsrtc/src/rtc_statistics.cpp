    ...
    for(auto ssrc_vs_id: media_ssrc_vs_id_) {
		...
		if (media_stats != NULL) {
            ...
            if(*media_stats->media_type == "audio") {
                ...
                net_stats_[ssrc_vs_id.second].audio_stats.bitrate_bps = (&media_stats->bytes_sent)->is_defined() ? *media_stats->bytes_sent : 0;
            }

            if(*media_stats->media_type == "video") {
                net_stats_[ssrc_vs_id.second].video_stats.sourceid = ssrc_vs_id.second;
                net_stats_[ssrc_vs_id.second].video_stats.width = (&media_stats->frame_width)->is_defined() ? *media_stats->frame_width : 0;
                net_stats_[ssrc_vs_id.second].video_stats.height = (&media_stats->frame_height)->is_defined() ? *media_stats->frame_height : 0;
                net_stats_[ssrc_vs_id.second].video_stats.bitrate_bps = (&media_stats->bytes_sent)->is_defined() ? *media_stats->bytes_sent : 0;
                net_stats_[ssrc_vs_id.second].video_stats.fps = (&media_stats->frames_per_second)->is_defined() ? *media_stats->frames_per_second : 0;
                net_stats_[ssrc_vs_id.second].video_stats.loss_rate = 0;
                net_stats_[ssrc_vs_id.second].video_stats.delay_ms = (&media_stats->total_packet_send_delay)->is_defined() ? *media_stats->total_packet_send_delay - net_stats_[ssrc_vs_id.second].video_stats.delay_ms : 0;
                net_stats_[ssrc_vs_id.second].video_stats.key_frame_count = (&media_stats->key_frames_encoded)->is_defined() ? *media_stats->key_frames_encoded : 0;
                net_stats_[ssrc_vs_id.second].video_stats.fir_count = (&media_stats->fir_count)->is_defined() ? *media_stats->fir_count : 0;
                net_stats_[ssrc_vs_id.second].video_stats.pli_count = (&media_stats->pli_count)->is_defined() ? *media_stats->pli_count : 0;
                net_stats_[ssrc_vs_id.second].video_stats.nack_count = (&media_stats->nack_count)->is_defined() ? *media_stats->nack_count : 0;
                net_stats_[ssrc_vs_id.second].video_stats.codec_name = (&media_stats->encoder_implementation)->is_defined() ? *media_stats->encoder_implementation : 0;
            }
