...
// update encoder framerate without recreating the context
int nvmpi_encoder_set_fps(nvmpictx* ctx, unsigned int fps);
// update encoder bitrate without recreating the context
int nvmpi_encoder_set_bitrate(nvmpictx* ctx, unsigned int bitrate);
// force the next frame to be encoded as IDR
int nvmpi_encoder_force_idr(nvmpictx* ctx);
// reconfigure resolution while keeping the encoder instance alive
