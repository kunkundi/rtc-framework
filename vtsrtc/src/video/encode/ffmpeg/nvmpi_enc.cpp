...
#define TEST_ERROR(condition, message, errorCode)            \
  do {                                                       \
    if (condition) {                                         \
      std::cerr << message << " Error code: " << errorCode   \
                << std::endl;                                \
      return errorCode;                                      \
    }                                                        \
  } while (0)
...
struct nvmpictx {
  ...
  bool runtime_vbv_update_supported;
...
constexpr int kOutputPlaneDqTimeoutMs = 5;

int nvmpi_apply_encoder_settings(nvmpictx* ctx);
int nvmpi_start_encoder_io(nvmpictx* ctx);
void nvmpi_stop_encoder_io(nvmpictx* ctx);
...
static bool encoder_capture_plane_dq_callback(struct v4l2_buffer* v4l2_buf,
  ...
  if (!pkt) {
    // TODO wait for user to read buffer. make send_frame return AVERROR(EAGAIN)
    // until avcodec_receive_packet() is called
    // TODO pass warning to avlog
    printf(
        "[libnvmpi][W]: EAGAIN. User must read output. nvmpi encoder packet "
        "memory pool is empty! Packet will be dropped. There may be artifacts "
        "in the output video.\n");
  } else {
    pkt->pts = (v4l2_buf->timestamp.tv_usec % 1000000) +
               (v4l2_buf->timestamp.tv_sec * 1000000UL);
    // AV_PKT_FLAG_KEY 0x0001. if current packet is keyframe then
    // enc_metadata.KeyFrame should be 0x1, so it should be OK to just assign
    // value
...
static int setup_output_dmabuf(nvmpictx* ctx, uint32_t num_buffers) {
  ...
  for (uint32_t i = 0; i < ctx->enc->output_plane.getNumBuffers(); i++) {
    ...
    NVBUF_COLOR_FORMAT_YUV420_709; break; case V4L2_COLORSPACE_SMPTE170M:
        default:
            cParams.colorFormat = ctx->enable_extended_colorformat ?
    ...
    else if (ctx->encoder_pixfmt == V4L2_PIX_FMT_H265)
    {
        ...
        if (ctx->profile == V4L2_MPEG_VIDEO_H265_PROFILE_MAIN10 &&
    (ctx->bit_depth == 10))
        {
...
int nvmpi_apply_encoder_settings(nvmpictx* ctx) {
  ...
  if (ctx->enableLossless) {
    ...
    if (ctx->ratecontrol == V4L2_MPEG_VIDEO_BITRATE_MODE_VBR) {
      uint32_t peak_bitrate =
          ctx->peak_bitrate < ctx->bitrate
              ? static_cast<uint32_t>(1.2f * ctx->bitrate)
              : ctx->peak_bitrate;
  ...
  if (ctx->qmax > 0 || ctx->qmin > 0) {
    ret = ctx->enc->setQpRange(ctx->qmin, ctx->qmax, ctx->qmin, ctx->qmax,
                               ctx->qmin, ctx->qmax);
    TEST_ERROR(ret < 0, "Could not set encoder qp range", ret);
  }
...
int nvmpi_start_encoder_io(nvmpictx* ctx) {
  ...
  ret = ctx->enc->output_plane.setupPlane(V4L2_MEMORY_MMAP, ctx->packets_num,
                                          true, false);
#else
  ...
  ret = ctx->enc->capture_plane.setupPlane(V4L2_MEMORY_MMAP, ctx->packets_num,
                                           true, false);
...
nvmpictx* nvmpi_create_encoder(nvEncParam* param) {
  ...
  ctx->max_perf = true;       // TODO invistigate why encoder is slow without
                         // max_perf even with MAXN power mode
  ctx->vbv_buffer_size = param->vbv_buffer_size;
  ctx->runtime_vbv_update_supported = true;
  ...
  ret = ctx->enc->setCapturePlaneFormat(ctx->encoder_pixfmt, ctx->width,
                                        ctx->height, NVMPI_ENC_CHUNK_SIZE);
  ...
  ret = nvmpi_apply_encoder_settings(ctx);
  TEST_ERROR_NULL(ret < 0, "Could not apply encoder settings", ret);

  ret = nvmpi_start_encoder_io(ctx);
  TEST_ERROR_NULL(ret < 0, "Could not start encoder io", ret);
...
int nvmpi_encoder_put_frame(nvmpictx* ctx, nvFrame* frame) {
  ...
  if (ctx->index < ctx->enc->output_plane.getNumBuffers()) {
    ...
    ret = ctx->enc->output_plane.mapOutputBuffers(
        v4l2_buf, ctx->output_plane_fd[v4l2_buf.index]);
    if (ret < 0) {
      cerr << "Error while mapping buffer at output plane" << endl;
    }
#endif
  } else {
    ret = ctx->enc->output_plane.dqBuffer(v4l2_buf, &nvBuffer, NULL,
                                          kOutputPlaneDqTimeoutMs);
    if (ret < 0) {
      return -EAGAIN;
    }
  }
  ...
  for (uint32_t j = 0; j < nvBuffer->n_planes; j++) {
    ...
    if (ret < 0) {
      cerr << "Error while NvBufSurfaceSyncForDevice at output plane for "
              "V4L2_MEMORY_DMABUF"
           << endl;
    }
#else
    ret = NvBufferMemSyncForDevice(nvBuffer->planes[j].fd, j,
                                   (void**)&nvBuffer->planes[j].data);
    if (ret < 0) {
      cerr << "Error while NvBufferMemSyncForDevice at output plane for "
              "V4L2_MEMORY_DMABUF"
           << endl;
    }
#endif
...
int nvmpi_encoder_set_fps(nvmpictx* ctx, unsigned int fps) {
  ...
  if (ctx->fps_n == fps && ctx->fps_d == 1) {
    return 0;
  }
  ctx->fps_n = fps;
  ctx->fps_d = 1;
  return ctx->enc->setFrameRate(ctx->fps_n, ctx->fps_d);
}
...
int nvmpi_encoder_set_bitrate(nvmpictx* ctx, unsigned int bitrate) {
  ...
  if (ctx->vbv_buffer_size && ctx->runtime_vbv_update_supported) {
    // Runtime VBV updates are not supported on some Jetson stacks once the
    // stream is running. Keep bitrate updates effective even if VBV ioctl
    // fails after start.
    if (ctx->vbv_buffer_size < ctx->bitrate) {
      ctx->vbv_buffer_size = ctx->bitrate;
    }
    ret = ctx->enc->setVirtualBufferSize(ctx->vbv_buffer_size);
    if (ret < 0) {
      ctx->runtime_vbv_update_supported = false;
      cerr << "[libnvmpi][W]: runtime setVirtualBufferSize unsupported, disable "
              "future VBV runtime updates and continue. ret="
           << ret << endl;
    }
  }

  if (ctx->ratecontrol == V4L2_MPEG_VIDEO_BITRATE_MODE_VBR) {
    const uint32_t peak_bitrate =
        ctx->peak_bitrate < ctx->bitrate
            ? static_cast<uint32_t>(1.2f * ctx->bitrate)
            : ctx->peak_bitrate;
    ret = ctx->enc->setPeakBitrate(peak_bitrate);
...
int nvmpi_encoder_reconfigure(nvmpictx* ctx,
                              unsigned int width,
                              unsigned int height) {
  if (!ctx || !ctx->enc || width == 0 || height == 0) {
    return -1;
  }
  ...
  TEST_ERROR(ret < 0, "Could not set output plane format during reconfigure",
             ret);
  ...
  ret = nvmpi_apply_encoder_settings(ctx);
  TEST_ERROR(ret < 0, "Could not apply encoder settings during reconfigure",
             ret);

  ret = nvmpi_start_encoder_io(ctx);
  TEST_ERROR(ret < 0, "Could not start encoder io during reconfigure", ret);

  return nvmpi_encoder_force_idr(ctx);
}
