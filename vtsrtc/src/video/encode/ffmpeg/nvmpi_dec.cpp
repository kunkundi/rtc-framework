...
struct nvmpictx {
  ...
  unsigned int
      frame_linedatasize[MAX_NUM_PLANES];  // usable data size for 1 line
  ...
  // get dst_dma buffer params and set corresponding frame size and linesize in
  // nvmpictx
...
void nvmpictx::initDecoderCapturePlane(v4l2_format& format) {
  ...
  TEST_ERROR(ret < 0,
             "Error while getting value of minimum capture plane buffers", ret);
  ...
  ret = NvBufSurf::NvAllocate(&cParams, numberCaptureBuffers,
                              dmaBufferFileDescriptor);
  TEST_ERROR(ret < 0, "Failed to create buffers", error);
  for (int index = 0; index < numberCaptureBuffers; index++) {
    ret = NvBufSurfaceFromFd(dmaBufferFileDescriptor[index],
                             (void**)(&(dmaBufferSurface[index])));
...
void nvmpictx::deinitDecoderCapturePlane() {
  ...
  for (int index = 0; index < numberCaptureBuffers;
       index++)  // V4L2_MEMORY_DMABUF
  {
...
#ifdef WITH_NVUTILS
    frame_linedatasize[i] =
        parm.width[i] * parm.bytesPerPix[i];  // valid only for nvutils
#else
...
void nvmpictx::initFramePool() {
  // if(bufNumber <= 0) return false; //TODO log msg //TODO check if it's
  // already allocated and deinit first
  NvBufferColorFormat cFmt = out_pixfmt == NV_PIX_NV12
...
void respondToResolutionEvent(v4l2_format& format, v4l2_crop& crop,
  ...
  ctx->output_height =
      ctx->resized.height ? ctx->resized.height : crop.c.height;
  ...
  // alloc frame pool buffers (dst_dma buffers). TODO: check if already
  // allocated and deinit pool first
  ctx->initFramePool();
  // get dst_dma buffer params and set corresponding frame size and linesize in
  // nvmpictx
...
void dec_capture_loop_fcn(void* arg) {
  ...
  while (!(ctx->eos || dec->isInError())) {
    ...
    while (!ctx->eos) {
      ...
      if (fb) {
#ifdef WITH_NVUTILS
        ret = NvBufSurfTransform(ctx->dmaBufferSurface[v4l2_buf.index],
                                 fb->dst_dma_surface, &(ctx->transform_params));
#else
        ret = NvBufferTransform(dec_buffer->planes[0].fd, fb->dst_dma_fd,
                                &(ctx->transform_params));
#endif
        TEST_ERROR(ret == -1, "Transform failed", ret);
        fb->timestamp = (v4l2_buf.timestamp.tv_usec % 1000000) +
...
// TODO: accept in nvmpi_create_decoder input stream params (width and height,
// etc...) from ffmpeg.
