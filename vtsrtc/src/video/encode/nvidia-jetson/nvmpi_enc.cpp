...
nvmpictx *nvmpi_create_encoder(nvCodingType codingType, nvEncParam *param) {
  ...
  switch (ctx->profile) {
    ...
    default: { ctx->raw_pixfmt = V4L2_PIX_FMT_YUV420M; }
  }
  ...
  if (ctx->qmax > 0 || ctx->qmin > 0) {
    ctx->enc->setQpRange(ctx->qmin, ctx->qmax, ctx->qmin, ctx->qmax, ctx->qmin,
                         ctx->qmax);
  }
  ...
  ret = ctx->enc->output_plane.setupPlane(V4L2_MEMORY_USERPTR, ctx->packets_num,
                                          false, true);
...
int nvmpi_encoder_get_packet(nvmpictx *ctx, nvPacket *packet) {
  ...
  if (ctx->packet_pools->empty()) return -1;
  while (ctx->packet_pools->empty()) {
  }
...
int nvmpi_encoder_set_fps(nvmpictx *ctx, unsigned int fps) {
  int ret = ctx->enc->setFrameRate(fps, 1);
  TEST_ERROR(ret < 0, "Could not set framerate", ret);
...
int nvmpi_encoder_set_bitrate(nvmpictx *ctx, unsigned int bitrate) {
  int ret = ctx->enc->setPeakBitrate(bitrate);
  TEST_ERROR(ret < 0, "Could not set bitrate", ret);
...
int nvmpi_encoder_force_idr(nvmpictx *ctx) {
  ...
  TEST_ERROR(ret < 0, "Could not forceIDR", ret);
  return 0;
}