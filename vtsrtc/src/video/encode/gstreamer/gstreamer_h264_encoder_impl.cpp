#include "gstreamer_h264_encoder_impl.h"
...
std::atomic<uint64_t> g_gstreamer_h264_encoder_instance_id{0};
std::once_flag g_gstreamer_init_once;
bool g_gstreamer_init_result = false;
...
bool EnsureGstreamerInitialized() {
  std::call_once(g_gstreamer_init_once, [] {
    GError* error = nullptr;
    g_gstreamer_init_result = gst_init_check(nullptr, nullptr, &error);
    if (!g_gstreamer_init_result) {
  ...
  return g_gstreamer_init_result;
}
...
GStreamerH264EncoderImpl::GStreamerH264EncoderImpl(
    const cricket::VideoCodec& codec, const vts_rtc::RtcConfig& rtc_config)
    : rtc_config_(rtc_config),
      instance_id_(g_gstreamer_h264_encoder_instance_id.fetch_add(1) + 1) {
...
GStreamerH264EncoderImpl::~GStreamerH264EncoderImpl() { Release(); }

int GStreamerH264EncoderImpl::InitEncode(
    const VideoCodec* codec_settings, const VideoEncoder::Settings& settings) {
  LOG_INFO("[WEBRTC] Init jetson-gstreamer H264 encoder");
...
int32_t GStreamerH264EncoderImpl::Release() {
  std::lock_guard<std::mutex> lock(encoder_mutex_);
...
int32_t GStreamerH264EncoderImpl::RegisterEncodeCompleteCallback(
...
void GStreamerH264EncoderImpl::SetRates(
  ...
  if (!std::isfinite(framerate_fps) || framerate_fps < 1.0 ||
      bitrate_bps < 1) {
    LOG_WARN(
        "[WEBRTC] jetson-gstreamer SetRates failed because framerate or "
  ...
  if (bitrate_changed) {
    ...
    if (g_object_class_find_property(encoder_class, "bitrate") != nullptr) {
      g_object_set(G_OBJECT(encoder_element_), "bitrate",
                   static_cast<guint>(bitrate_bps_), nullptr);
    } else if (!has_logged_restart_warning_) {
      LOG_WARN(
          "[WEBRTC] jetson-gstreamer encoder has no runtime bitrate property; "
...
int32_t GStreamerH264EncoderImpl::Encode(
  ...
  if ((frame_width & 1u) != 0 || (frame_height & 1u) != 0) {
    LOG_ERROR(
        "[WEBRTC] jetson-gstreamer H264 encoder only supports even "
        "resolution, got <%ux%u>",
  ...
  if (encoded_image_callback_ == nullptr) {
    if (!has_reported_missing_callback_) {
      LOG_WARN(
          "[WEBRTC] jetson-gstreamer Encode() was called before "
  ...
  if (pending_frames_.size() > kMaxPendingFrames) {
    LOG_WARN(
        "[WEBRTC] jetson-gstreamer pending frame queue overflow (%zu), "
...
VideoEncoder::EncoderInfo GStreamerH264EncoderImpl::GetEncoderInfo() const {
  EncoderInfo info;
  info.supports_native_handle = false;
  info.implementation_name = "jetson-gstreamer-h264";
...
void GStreamerH264EncoderImpl::SetFecControllerOverride(
    FecControllerOverride* fec_controller_override) {}

void GStreamerH264EncoderImpl::OnPacketLossRateUpdate(float packet_loss_rate) {}

void GStreamerH264EncoderImpl::OnRttUpdate(int64_t rtt_ms) {}

void GStreamerH264EncoderImpl::OnLossNotification(
    const LossNotification& loss_notification) {}

bool GStreamerH264EncoderImpl::InitializePipelineLocked(unsigned int width,
                                                        unsigned int height) {
...
void GStreamerH264EncoderImpl::DestroyPipelineLocked() {
  pending_frames_.clear();
  ready_packets_.clear();
...
bool GStreamerH264EncoderImpl::ReinitializePipelineLocked(unsigned int width,
                                                          unsigned int height) {
  DestroyPipelineLocked();
  return InitializePipelineLocked(width, height);
}

bool GStreamerH264EncoderImpl::WriteFrameToPipeLocked(
...
bool GStreamerH264EncoderImpl::PullPacketFromSinkLocked(
...
int GStreamerH264EncoderImpl::DrainPacketsLocked(int first_wait_timeout_ms) {
  if (appsink_ == nullptr) {
    return -1;
  }
...
int GStreamerH264EncoderImpl::DeliverPacketLocked(const VideoFrame& frame,
  ...
  if (encoded_image_callback_ == nullptr) {
    LOG_ERROR(
        "[WEBRTC] jetson-gstreamer encode callback is null while delivering "
        "packet");
...
bool GStreamerH264EncoderImpl::RequestKeyFrameLocked() {
  // Keep this best-effort mode simple in experimental native-gstreamer path.
  // Keyframe hints are currently satisfied by GOP policy.
  return true;
}

bool GStreamerH264EncoderImpl::LaunchPipelineLocked(unsigned int width,
                                                    unsigned int height) {
  const unsigned int fps = std::max(1u, fps_);
  const unsigned int profile = MapProfileToGstLaunchProfileValue(profile_);
  ...
  pipeline_desc << "appsrc name=src is-live=true block=true format=time "
                ...
                << " iframeinterval=" << std::max(1u, gop_size_)
                << " idrinterval=" << std::max(1u, gop_size_)
  ...
  if (pipeline_ == nullptr || error != nullptr) {
    const char* message = (error != nullptr && error->message != nullptr)
                              ? error->message
                              : "unknown";
    LOG_ERROR("[WEBRTC] gst_parse_launch failed: %s", message);
    if (error != nullptr) {
      g_error_free(error);
    }
    DestroyPipelineLocked();
...
void GStreamerH264EncoderImpl::MaybeLogHelperExitLocked(const char* context) {
  if (pipeline_bus_ == nullptr) {
    return;
  }
...
void GStreamerH264EncoderImpl::InitializeResolutionBitrateLimits() {
...
int GStreamerH264EncoderImpl::MapProfileToGstLaunchProfileValue(
...
void GStreamerH264EncoderImpl::ReportInit() {
...
void GStreamerH264EncoderImpl::ReportError() {
